<script>
	...
	import './styles.css';
	const routesDirArr = [
		'Chart','Market','Exch','Board','Office','Sys'
	];
</script>
...
<div id="site-wrap">
	<nav>
		...
		{#each routesDirArr as routesDir, i}
			<li>
				<a class={$page.url.pathname === '/'+routesDir.toLowerCase() ? 'current' : undefined} href="/{routesDir==='Chart'?'':routesDir.toLowerCase()}">
					<div class="title-box">
						{#if i === 0}
							<ChartLine />
						{:else if i === 1}
...
<style>
	#site-wrap {
		max-width:1020px;width:100vw;margin:0 auto;
		min-height:100vh;
		position:relative;
	...
	main {
		flex-grow: 1;min-height:100%;
		padding-bottom:60px;background: var(--main-color);
		position:relative;
	}
	.container {
	...
	nav {
        min-width:220px;min-height:100%;
    ...
    #menu-box li {position:relative;list-style-type: none;}
    #menu-box li a {display:block;height:50px;line-height:50px;padding-right:15px;text-align: right;color:#d1d1d1;border-radius: 25px 0 0 25px;}
    #menu-box li a:hover {text-decoration: none;background:  rgba(73, 73, 245,0.3);color:#fff;}
    #menu-box li a.current {background:var(--header-color);color:#fff;}
    #menu-box li .title-box {display:flex;justify-content: space-between;align-items: center;padding-left:15px;font-size:1.5rem;}
	footer {
		position:absolute;bottom:0;left:0;right:0;
		background: var(--footer-color);height:50px;
		padding:15px;color:#d1d1d1;
	}
